...
Function WriteGame(no, vis, visitor, ho, home, fav, line, min, max, note, deadline)
	dim rv,passed,disabled,readonly,isDefaultDeadline
	...
	rv="<tr>"
	rv = rv &"<td align='center' valign='middle' class='game_no' rowspan='"&rs&"'>" & no & "</td>" & chr(13) & _
	...
	if not isDefaultDeadline then
		...
		rv = rv&">"&dl&"</td></tr>" '<input type='hidden' name='dl_"&no&"' value='"&deadline&"'>
	end if
	WriteGame = rv
End Function
...
<% End If %>
<tr><td colspan="7" class="warning" align="center"><hr noshade>The deadline to get your picks in is <%=g.defaultDeadline%>.&nbsp;&nbsp;&nbsp;Individual games may have earlier deadlines.</td></tr>

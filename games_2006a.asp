...
'	Dim picks, values, games(20), vis(20), visitors(20), hos(20), homes(20), favs(20), lines(20), mins(20), maxs(20)
	...
	Function WriteGame(no, vis, visitor, ho, home, fav, line, min, max)
		...
		if no = 11 then
			rv = "<td align='center' valign='middle' class='game_no'>" & no & "</td>" & chr(13) & _
					"<td align='left' valign='top'><input disabled='disabled' type='radio' name='' value='" & vis & "'"
			If u.GetPick(no-1) = vis Then
				rv = rv & " checked>"
			Else
				rv = rv & ">"
			End If
			rv = rv & "<input type='hidden' name='game_" & no & "' value='" & u.GetPick(no-1) & "'>"
		else
			rv = "<td align='center' valign='middle' class='game_no'>" & no & "</td>" & chr(13) & _
		...
		End If
		if no = 11 then
			rv = rv & "<BR><input disabled='disabled' type='radio' name='' value='" & ho & "'"
		else
	...
	Function WriteValue(no,min,max)
		Dim rv, i
		if no = 11 then
			rv = "<input type='text' size='2' readonly='readonly' value='" & CInt(u.GetValue(no-1)) & "' name='val_" & no & "' >"
		else

...
<%
    ...
    sub setDeadlines()
	    ...
	    for i=1 to num_games
		    params(0)=i
		    sql="select dtiKickOff k from dbo.pkmGames where Year='2014' and Week=15 AND Game=?"
	...
	if now()>cdate("1/12/2015  07:29:59 PM") then
		Response.Write("<font color=red><center>Sorry but it's too late to make picks.</center></font><br>")
		Response.Write("<center><a href='bowl_Games_2014.asp?toolate=yes&uid=" & uid & "'>Back to Games of the Week</a></center>")
	Else
		Connect
		setDeadlines
		LoadPicks
		'DeletePicks uid, 15, 2014
		InsertPicksBowls uid, Picks, Values, 15, 2014,deadlines
		'WritePicks
		Disconnect
		Response.Redirect("bowl_Games_2014.asp?uid=" & uid)
	End If
%>
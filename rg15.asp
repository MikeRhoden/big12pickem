	...
	<%For j = 0 to g.Game_Count	- 1%>
		...
		<%If u.GetPick(j) <> "" AND u.GetPick(j) <> "0" Then
			...
			<table border="0" cellpadding="1" cellspacing="0" align="center">
				<tr><td class="<%=pick_style%>"><span class="<%=correct_style%>"><%if j = g.Game_Count-1 AND 0 = 1 then%>???<%else%><%=u.GetPick(j)%><%end if%></span></td><td class="<%=val_style%>"><%=u.GetValue(j)%></td></tr>
			</table>
		<%Else%>&nbsp;<%End If%>
		</td>
	<%Next%>

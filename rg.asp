...
<!-- #include file="cClasses2_bowls.asp" --->
<html>
<head>
...
<%
	Function GetUsers(wk)
		...
		rs.Open "Select Distinct pkmUsers.UID, Last_Name, First_Name, Week_Score_2005_" & wk & " From pkmUsers, pkmPicks Where pkmUsers.UID = pkmPicks.UID And Week = '" & wk & "' Order By Week_Score_2005_" & wk & " DESC, Last_Name, First_Name ASC"
...
<%For i = 0 to UBound(users) - 1
		...
		Else
			bg = "#ffffff"
		End If
		If u.Week_Credits > 250 Then
			total_style = "value_total_pos"
		ElseIf u.Week_Credits < 250 Then

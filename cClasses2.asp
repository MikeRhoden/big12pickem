...
Class cUser
	...
	Private cn, rs, cd
	Private uid, f_name, l_name, email, picks(20), values(20), correct_picks(20), weekly_credits
	...
	Private Sub CalcCorrectPicks(week,yr)
		Dim g, i, winner
		Set g = New cGames
...
'	Private uid, f_name, l_name, email, picks(20), values(20)
	...
	Public Sub SetWeekCredits(wk,yr)
	    if cint(yr)<2012 then
		    ...
		    cd.CommandText = "Update dbo.pkmScores Set Score = " & weekly_credits & " Where UID = '" & uid & "' and Year = '" & yr & "' AND Week = " & wk
	    end if
		cd.Execute
	End Sub
	public sub updateMandatoryCorrect(tb,wk,yr)
	    if cint(yr)<2012 then
		    cd.CommandText = "Update pkmUsers Set Week_Mandatory_Correct_" & yr & "_" & wk & " = " & tb & " Where UID = '" & uid & "'"
	    else
		    cd.CommandText = "Update dbo.pkmScores Set Mandatory_Correct = " & tb & " Where UID = '" & uid & "' and Year = '" & yr & "' AND Week = " & wk
	    end if		
		cd.Execute
	end sub
End Class
...
Class cGames
	...
	Public Sub Initialize(wk,y)
		...
		Next
		For i = 0 to 11
			mins(i) = min_mandatory_bet
		Next
		For i = 12 to total_games - 1
			mins(i) = 0
		Next
		rs.close
	End Sub

<%@ language=vbscript%>
<!-- #include file="cClasses_bowls2006.asp" --->
<html>
<head>
<!-- #include file="funcs.asp" --->
<%
	Dim u, g, uid, toolate, cnPick,  i, remaining, max_credits, week, y, numGames
	Dim games(), vis(), visitors(), hos(), homes(), favs(), lines(), mins(), maxs()
	Set u = new cUser
	week = 14
	y = 2006
...
<script language="javascript">
	...
	function clearGame(num) {
		document.games["val_" + num].value = 5;
...
<form name="games" method=post action="postbowlgames_2006.asp?uid=<%=uid%>" onSubmit="return verify(this)">
...
<table border=0 width=100%>
	...
	<td align="left"><span class="warning">Once you submit your picks you can come back here & edit your picks until DEC 22 @ 6PM </span></td>
	</tr>
</table>
...
<%
	If Not toolate Then
%>
			Click Button to <input type=submit value="<%If u.GetPick(0)="0" Then%>Make<%Else%>Change<%End If%> Picks" disabled="disabled">
<%
	Else
%>

...
<%
    ...
    sub setDeadlines()
	    ...
	    for i=1 to num_games
		    params(0)=i
		    sql="select dtiKickOff k from dbo.pkmGames where Year='2015' and Week=15 AND Game=?"
	...
	if now()>cdate("1/11/2016 07:00:00 PM") then
		Response.Write("<font color=red><center>Sorry but it's too late to make picks.</center></font><br>")
		Response.Write("<center><a href='bowl_Games_2015.asp?toolate=yes&uid=" & uid & "'>Back to Games of the Week</a></center>")
	Else
		Connect
		setDeadlines
		LoadPicks
		InsertPicksBowls uid, Picks, Values, 15, 2015,deadlines
		Disconnect
		Response.Redirect("bowl_Games_2015.asp?uid=" & uid)
	End If
%>
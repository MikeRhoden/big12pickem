...
<!-- #include file="cClasses15_bowls.asp" --->
<html>
<head>
...
<%For i = 0 to UBound(users) - 1
	...
	<%For j = 0 to g.Game_Count	- 1%>
		...
		<%If u.GetPick(j) <> "" AND u.GetPick(j) <> "0" Then
				If j = g.Game_Count-1 AND 0=1 Then
					pick_style = "no_grid"
					val_style = "value_no"
				ElseIf u.GetPick(j) = g.GetVis(j) Then
					pick_style = "vis_grid"
					val_style = "value_vis"
			...
			<table border="0" cellpadding="1" cellspacing="0" align="center">
				<tr><td class="<%=pick_style%>"><span class="<%=correct_style%>"><%if j = g.Game_Count-1 AND 0=1 then%>???<%else%><%=u.GetPick(j)%><%end if%></span></td><td class="<%=val_style%>"><%if j = g.Game_Count-1 AND 0=1 then%>???<%else%><%=u.GetValue(j)%><%end if%></td></tr>
			</table>
		<%Else%>&nbsp;<%End If%>
		</td>
	<%Next%>

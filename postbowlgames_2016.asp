...
<%
    ...
    sub setDeadlines()
	    ...
	    for i=1 to num_games
		    params(0)=i
		    sql="select dtiKickOff k from dbo.pkmGames where Year='2016' and Week=15 AND Game=?"
	...
	if now()>cdate("1/9/2017 06:30:00 PM") then
		Response.Write("<font color=red><center>Sorry but it's too late to make picks.</center></font><br>")
		Response.Write("<center><a href='bowl_Games_2016.asp?toolate=yes&uid=" & uid & "'>Back to Games of the Week</a></center>")
	Else
		Connect
		setDeadlines
		LoadPicks
		InsertPicksBowls uid, Picks, Values, 15, 2016,deadlines
		Disconnect
		Response.Redirect("bowl_Games_2016.asp?uid=" & uid)
	End If
%>